...
void action::buy(ENetEvent& event, const std::string& header)
{
    ...
    else if (pipes[3] == "locks") 
    {
        packet::create(*event.peer, false, 0, 
        {
            ...
            std::format(
                ...
                /* restock items */
                "add_button|World Lock_242_2000_1|`oWorld Lock``|interface/large/store_buttons/store_buttons.rttex|`2You Get:`` 1 World Lock.<CR><CR>`5Description:`` Become the undisputed ruler of your domain with one of these babies.  It works like a normal lock except it locks the `$entire world``!  Won't work on worlds that other people already have locks on. You can even add additional normal locks to give access to certain areas to friends. `5It's a perma-item, is never lost when destroyed.``  `wRecycles for 200 Gems.``<CR><CR>Available: 100/100``<CR>Restock in: `59 Hours 9 Mins``|0|7|2000|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
                ...
                /* restock items */
                "add_button|Small Lock_202_50_1|`oSmall Lock``|interface/large/store_buttons/store_buttons.rttex|`2You Get:`` 1 Small Lock.<CR><CR>`5Description:`` Protect up to `$10`` tiles.  Can add friends to the lock so others can edit that area as well. `5It's a perma-item, is never lost when destroyed.``<CR><CR>Available: 100/100``<CR>Restock in: `59 Hours 9 Mins``|1|3|50|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
                ...
                "add_button|Big Lock_204_200_1|`oBig Lock``|interface/large/store_buttons/store_buttons.rttex|`2You Get:`` 1 Big Lock.<CR><CR>`5Description:`` Protect up to `$48`` tiles.  Can add friends to the lock so others can edit that area as well. `5It's a perma-item, is never lost when destroyed.``<CR><CR>Available: 100/100``<CR>Restock in: `59 Hours 9 Mins``|1|1|200|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
                ...
                /*          name,          id,  cost, amount                                                                                                                                                                                                                                                                                                                                                             */
                "add_button|Builder's Lock_4994_50000_1|`oBuilder's Lock``|interface/large/store_buttons/store_buttons17.rttex|`2You Get:`` 1 Builders Lock.<CR><CR>`5Description:`` Protect up to `$200`` tiles. Wrench the lock to limit it - it can either only allow building, or only allow breaking! `5It's a perma-item, is never lost when destroyed.``|0|2|50000|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
    ...
    else if (pipes[3] == "itempack")
    {
        packet::create(*event.peer, false, 0, 
        {
            ...
            /*          name,           id,  cost, amount                                                                                                                                                                                                                                                                                                              */
            "add_button|Small Seed Pack_5706_100_1|`oSmall Seed Pack``|interface/large/store_buttons/store_buttons.rttex|`2You Get:`` 1 Small Seed Pack.<CR><CR>`5Description:`` Contains one Small Seed Pack. Open it for `$5`` randomly chosen seeds, including 1 rare seed! Who knows what you'll get?!|1|4|100|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
            "add_button|Small Seed Pack Collection_5706_1000_10|`oSmall Seed Pack Collection``|interface/large/store_buttons/store_buttons18.rttex|`2You Get:`` 10 Small Seed Packs.<CR><CR>`5Description:`` Open each one for `$5`` randomly chosen seeds apiece, including 1 rare seed per pack! Who knows what you'll get?!|0|4|1000|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
            ...
            // https://growtopia.fandom.com/wiki/Gangland_Style
            "add_button|Gangland Style_462,464,466,468,474,476,470,472,480,478#10_5000_1|`oGangland Style``|interface/large/store_buttons/store_buttons2.rttex|`2You Get:`` 1 Fedora, 1 Dames Fedora, 1 Pinstripe Suit with Pants, 1 Flapper Headband with Dress, 1 Cigar, 1 Tommy Gun, 1 Victola and 10 Art Deco Blocks .<CR><CR>`5Description:`` Step into the 1920's with a Complete Outfit, a Tommygun, a Victrola that plays jazz music, and 10 Art Deco Blocks. It's the whole package!|0|6|5000|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
            
            // https://growtopia.fandom.com/wiki/Racing_Action_Pack
            "add_button|Racing Action Pack_492,494,490,488,356,496,428,430,410#2,482#2,484,48_3500_1|`oRacing Action Pack``|interface/large/store_buttons/store_buttons2.rttex|`2You Get:`` 1 Racing Start Flag, 1 Racing End Flag, 2 Checkpoints, 2 Big Old Sideways Arrows, 1 Big Old Up Arrow, 1 Big Old Down Arrow, 1 WristBand, 1 HeadBand, 1 Sports Ball Jersey and 1 Air Robinsons.<CR><CR>`5Description:`` Get all you need to host races in your worlds! You'll win the races too, with new Air Robinsons that make you run faster!|0|7|3500|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
            
            // https://growtopia.fandom.com/wiki/Composer's_Pack
            "add_button|Composer's Pack_412#20,420#20,422,424,426#20,414#20,416,418_5000_5|`oComposer's Pack``|interface/large/store_buttons/store_buttons3.rttex|`2You Get:`` 20 Sheet Music: Blank, 20 Sheet Music: Piano Note, 20 Sheet Music: Bass Note, 20 Sheet Music Drums, 5 Sheet Music: Sharp Piano, 5 Sheet Music: Flat Piano, 5 Sheet Music: Flat Bass and 5 Sheet Music: Sharp Bass .<CR><CR>`5Description:`` With these handy blocks, you'll be able to compose your own music, using your World-Locked world as a sheet of music. Requires a World Lock (sold separately!).|0|0|5000|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
            
            // https://growtopia.fandom.com/wiki/Fantasy_Pack
            "add_button|Fantasy Pack_600,594,604,608#3,606#10,598#3,596#3,583_5000_1|`oFantasy Pack``|interface/large/store_buttons/store_buttons3.rttex|`2You Get:`` 1 Mystical Wizard Hat Seed, 1 Wizards Robe, 1 Golden Sword, 1 Elvish Longbow, 10 Barrels, 3 Tavern Signs, 3 Treasure Chests and 3 Dragon Gates.<CR><CR>`5Description:`` Hear ye, hear ye! It's a pack of magical wonders!|0|6|5000|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
            
            // https://growtopia.fandom.com/wiki/School_Uniforms
            "add_button|Education Pack_674,652#3,650#10,654#20,658,676,670#10,672,?8870`8872`8874`8876#1,678_5000_1|`oEducation Pack``|interface/large/store_buttons/store_buttons4.rttex|`2You Get:`` 10 ChalkBoards, 3 School Desks, 20 Red Bricks, 1 Bulletin Board, 10 Pencils, 1 Growtopia Lunchbox, 1 Grey Hair Bun, 1 Apple and 1 Random School Uniform Item.<CR><CR>`5Description:`` If you want to build a school in Growtopia, here's what you need!|0|0|5000|0|||-1|-1||-1|-1||1||||||0|0|CustomParams:|\n"
    ...
    else 
    {
        ...
        if (!number(embed[2]) || !number(embed[3])) return;
        signed price = stoi(embed[2]);
        short amount = stoi(embed[3]);

        std::string received_list{};
        std::vector<std::string> id_list{};
        std::vector<item> item_list{};
        if (embed[1].contains(',')) id_list = readch(std::move(embed[1]), ',');
        else item_list.emplace_back(items[stoi(embed[1])]);
        
        for (std::string &id : id_list)
            if (number(id)) 
            {
                ::item &item = items[stoi(id)];
                item_list.emplace_back(item);
                peer->emplace(slot(stoi(id), amount)); received_list += std::format("{} {}, ", amount, item.raw_name);
            }
            else
            {
                std::vector<std::string> custom_amount = readch(std::move(id), '#');
                ::item item;
                if (custom_amount[0].contains("?")) // @todo
                {
                    std::string remove_qm = custom_amount[0].substr(1, custom_amount[0].length()-1); // @todo
                    std::vector<std::string> choices = readch(std::move(remove_qm), '`');
                    item = items[stoi(choices[(rand() % choices.size()-1)])];
                }
                else item = items[stoi(custom_amount[0])];
                short c_amount = stoi(custom_amount[1]);
                item_list.emplace_back(item);
                peer->emplace(slot(item.id, c_amount)); received_list += std::format("{} {}, ", c_amount, item.raw_name);
            }
        ...
        packet::create(*event.peer, false, 0, 
        {
            ...
            std::format(
                ...
                embed[0], price, peer->gems -= price, received_list
            ).c_str()
        });
        on::SetBux(event);
        inventory_visuals(event);
