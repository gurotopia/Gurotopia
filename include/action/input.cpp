...
void action::input(ENetEvent& event, const std::string& header)
{
    ...
    else 
    {
        ...
        std::string player_chat = std::format("CP:0_PL:0_OID:_player_chat={}{}", chat_color, text);
        std::string message = std::format("CP:0_PL:0_OID:_CT:[W]_ `6<`{}{}``>`` `$`${}{}````", pPeer->prefix, pPeer->ltoken[0], chat_color, text);
